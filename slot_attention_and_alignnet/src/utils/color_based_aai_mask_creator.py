...
def get_true_masks_from_inp(inp):
    """Return all masks for input imgs"""
    preprocessed_imgs = np.ceil(((inp / 2.0) + 0.5) * 255.0).astype(np.uint8)
    return jnp.asarray(
        [get_ground_truth_masks(img, MAX_MASKS) for img in preprocessed_imgs]
    )
