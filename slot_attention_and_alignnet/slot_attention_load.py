...
run_name = "sa_sprite_run_4_resume"
config_name = "slot_attention_clevr.yaml"
debug = True
model_class = SlotAttentionAE  # AlignedSlotAttention
...
def main():
    ...
    # ds_train = CLEVRHansLoader(data_path, cfg, split="train", variant=3)
    # ds_val = CLEVRHansLoader(data_path, cfg, split="validation", variant=3)
    ...
    with open(
        f"./slot_attention_and_alignnet/runs/sa_sprite_run_4/model/params_120000.pkl",
        "rb",
    ) as f:
        params, state = pickle.load(f)
    with open(
        f"./slot_attention_and_alignnet/runs/sa_sprite_run_4/model/optimizer_120000.pkl",
        "rb",
    ) as f:
